...
*/	
	...
	uint8_t temp = (uint8_t) (I2C1->SR1 | I2C1->SR2);  // read SR1 and SR2 to clear the ADDR bit
}
...
void I2C_Read (uint8_t Address, uint8_t *buffer, uint8_t size){
	...
	// 1. if only one byte needs to be read
	if (size == 1){
		// step-1.1: write the slave Address and wait for the ADDR bit (bit 1 int SR1) to be set
		I2C1->DR = Address;	// send the addredd
		while(!((I2C1->SR1) & (1 << 1)));	// wait for ADDR bit to set
		
		// step-1.2: clear the ADDR bit by reading the SR1 and SR2 registers
		I2C1 -> CR1 &= ~(1U << 10);	// clear the ACK bit
		uint8_t temp = (uint8_t) (I2C1->SR1 | I2C1->SR2);	// read SR1 and SR2 to clear the ADDR bit .... EV6 condition
		I2C1->CR1 |= (1<<9);	// stop I2C
		
		// steep-1.3: wait for RXNE (receive buffer not empty) bit to set 
		while(!(I2C1->SR1 & (1<<6)));
	...
	// 2.	
		...
		while(remaining > 2){
			// step-2.3:
			while(!(I2C1->SR1 & (1 << 6)));	// wait for RNXE to set
